...
<script>
  export default {
    ...
    mounted () {
      ...
      this.transHideHei = this.getStyleAttr(this.$refs.trans, 'height')
    },
    methods: {
...
.visit-item {
  ...
  .hide-tips {
    ...
    .switch {
      ...
      top: -4px;   
      &.on {
        background: url('../assets/imgs/on-icon.png') no-repeat top left;
        background-size: 24px;
      }
      &.off {
        background: url('../assets/imgs/off-icon.png') no-repeat top left;
        background-size: 24px;
      }
    }
  }
